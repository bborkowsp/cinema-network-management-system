...
class MovieService implements MovieUseCases {
    ...
    public Page<MovieListResponse> getMovies(Pageable pageable) {
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " performed getMovies operation.");
        return movieRepository.findAll(pageable).map(movieMapper::mapMovieToMovieListResponse);
    }

    @Override
    public List<MovieResponse> getAllMovies() {
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " requested all movies.");
    ...
    public List<Genre> getGenres() {
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " requested all genres.");
        return List.of(Genre.values());
    }

    @Override
    public List<AgeRestriction> getAgeRestrictions() {
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " requested all age restrictions.");
        return List.of(AgeRestriction.values());
    }

    @Override
    public List<String> getMovieTitles() {
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " requested all movie titles.");
        final var movies = movieRepository.findAll();
    ...
    public MovieResponse getMovie(String title) {
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " requested movie with title " + title + ".");
    ...
    public void createMovie(MultipartFile image, CreateMovieRequest createMovieRequest) {
        ...
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " created movie with title " + createMovieRequest.title() + ".");
    }
    ...
    public void updateMovie(String title, MultipartFile image, UpdateMovieRequest updateMovieRequest) {
        ...
        movieRepository.save(movie);
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " updated movie with title " + title + ".");
    }
    ...
    public void deleteMovie(String title) {
        ...
        LOGGER.info("User with email " + userSecurityContextUseCases.getCurrentUserEmail() + " deleted movie with title " + title + ".");
    }
